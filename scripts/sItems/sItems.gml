...
function determinRare(){
	...
	rand = irandom_range(0,100);
	if rand <= 100-(global.roomsCleared/5) {
		item = 0;
...
function dropItem(){
	...
	if drop{
		...
		switch (rare){
			case 0:
				var itemIndex = irandom(array_length(global.commonItems) - 1);
                var commonItem = global.commonItems[itemIndex];
                instance_create(x, y, commonItem.object);
				break;
			case 1:
				var itemIndex = irandom(array_length(global.rareItems) - 1);
                var rareItem = global.rareItems[itemIndex];
                instance_create(x, y, rareItem.object);
				break;
			case 2:
				var itemIndex = irandom(array_length(global.epicItems) - 1);
                var epicItem = global.epicItem[itemIndex];
                instance_create(x, y, epicItem.object);
				break;
			case 3:
				var itemIndex = irandom(array_length(global.exoticItems) - 1);
                var exoticItem = global.exoticItem[itemIndex];
                instance_create(x, y, exoticItem.object);
				break;
		}
	}
}